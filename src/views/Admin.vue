<template>
  <div class="admin-page pt-16">
    <v-container class="py-8">
      ...
      <v-dialog v-model="showConfirmAdd" max-width="500">
        <v-card>
          <v-card-title class="text-h5 pa-6">
            <v-icon class="mr-3" color="primary">mdi-database-plus</v-icon>
            Confirmar creación
          </v-card-title>
          <v-card-text class="pa-6 pt-0">
            <p class="text-body-1 mb-4">
              ¿Deseas agregar el curso <strong>"{{ pendingNew?.nombre }}"</strong> a la base de datos?
            </p>
          </v-card-text>
          <v-card-actions class="pa-6 pt-0">
            <v-spacer />
            <v-btn variant="text" @click="showConfirmAdd=false">
              Cancelar
            </v-btn>
            <v-btn color="primary" @click="performAdd" :loading="loading">
              Agregar Curso
            </v-btn>
          </v-card-actions>
        </v-card>
...
const pendingNew = ref(null) // holds payload waiting for confirmation
...
// Create flow: receive payload from CourseForm but wait for explicit confirmation
const create = (payload) => {
  pendingNew.value = payload
